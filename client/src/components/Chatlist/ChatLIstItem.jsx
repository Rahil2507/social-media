...
function ChatListItem({data, isContactPage = false}) {
  ...
  const handleContactClick = async () => {
    // if(currentChatUser?.id === data?.id) {
      if(!isContactPage) {
        ...
        const {data: {users}} = await axios.get(`${GET_INITIAL_CONTACTS_ROUTE}/${userInfo.id}`)
  ...
  return <div onClick={handleContactClick} className="flex pb-2 cursor-pointer items-center hover:bg-background-default-hover">
    ...
    <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
      <div className="flex justify-between">
        ...
        {!isContactPage && <div>
          <span  className={`${data.totalUnreadMessage>0 ? "text-secondary" : "text-icon-green"}`}>
      ...
      <div className="flex border-b border-conversation-border pb-2 pt-1 pr-2">
        <div className="flex justify-between w-full">
          <span className="text-secondary line-clamp-1 text-sm">
            ...
            :<div className="flex items-center gap-1 max-w-[200px] md:max-w-[250px] md:max-w-[300px] lg:max-w-[200px] xl:max-w-[300px]">
