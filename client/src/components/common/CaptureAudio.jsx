...
useEffect(() => {
  if(recordedAudio) {
    const updatePlaybackTime = () => {
      setCurrentPlaybackTime(recordedAudio.curentTime)
    }
...
const sendRecording = async () => {
  ...
  return <div className="flex text-2xl w-full justify-end items-center">
    <div className="pt-1">
      <FaTrash onClick={() => hide(false)} className="text-panel-header-icon cursor-pointer text-xl" />
    </div>
    <div className="mx-4 py-2 px-4 text-white text-lg flex gap-3 justify-center items-center bg-search-input-container-background rounded-full drop-shadow-lg ">
      {isRecording ? <div className="text-red-500 animate-pulse w-60 text-center">
    ...
    <div>
      <MdSend onClick={sendRecording} className="text-panel-header-icon cursor-pointer mr-4" title="Send" />
    </div>
